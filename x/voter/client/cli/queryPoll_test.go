...
func networkWithPollObjects(t *testing.T, n int) (*network.Network, []*types.Poll) {
	...
	for i := 0; i < n; i++ {
		state.PollList = append(state.PollList, &types.Poll{Creator: "ANY", Id: uint64(i)})
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.Equal(t, tc.obj, resp.Poll)
			}
		})
	}
}
...
func TestListPoll(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			for j := i; j < len(objs) && j < i+step; j++ {
				assert.Equal(t, objs[j], resp.Poll[j-i])
			}
		}
	})
	...
	t.Run("Total", func(t *testing.T) {
		...
		require.Equal(t, objs, resp.Poll)
	})
}
